...
- `deepseek --upgrade` in detail: it resolves and validates the target version first; if the current runtime is healthy and already at least the target version, it reports "already latest" and exits without stopping the service or running npm; otherwise it stops the service, clears old DSH npx workspaces, syncs the global `dsh` command, and restarts in background. If the candidate fails because a plugin imports an export removed from DSH, it lists the incompatible plugins and asks whether to remove them from the `web` profile; after confirmation it runs the target version's `dsh plugin --profile web remove` command and retries the candidate; declining or failing the repair falls back through current, previous, or legacy usable runtimes (only runtimes that pass readiness validation are used for rollback).
