...
- `deepseek --upgrade` 的详细流程：先解析并校验目标版本，当前运行时健康且版本不低于目标版本时直接提示已是最新并退出（不停止服务、不执行 npm 操作）；否则停止服务 → 清理旧 DSH npx 工作区 → 同步全局 `dsh` 命令 → 重新后台启动。候选版本因插件依赖 DSH 已移除的导出，或插件清单与目标 DSH 契约不兼容（插件树加载失败）而启动失败时，会列出不兼容插件并询问是否从 `web` profile 移除；确认后使用目标版本的 `dsh plugin --profile web remove` 清理插件并重试候选运行时，拒绝或处理失败时依次尝试恢复当前、上一版或旧版可用运行时（只有通过就绪校验的运行时才会被用于回退）。
